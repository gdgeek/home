<template>
  <el-dialog :model-value="modelValue" :title="dialogTitle" width="420px" :close-on-click-modal="true"
    :close-on-press-escape="true" :append-to-body="true" :destroy-on-close="true" center class="login-modal"
    @update:model-value="(val) => emit('update:modelValue', val)" @closed="handleClosed">
    <!-- Logo -->
    ...
    <el-form ref="formRef" :model="loginForm" :rules="rules" label-position="top" size="large">
      <el-form-item :label="isSimplified ? '用户名' : '用戶名'" prop="username">
        <el-input v-model="loginForm.username" :placeholder="isSimplified ? '请输入用户名 / 手机号 / 邮箱' : '請輸入用戶名 / 手機號 / 郵箱'"
          :prefix-icon="User" clearable />
      </el-form-item>

      <el-form-item :label="isSimplified ? '密码' : '密碼'" prop="password">
        <el-input v-model="loginForm.password" type="password" :placeholder="isSimplified ? '请输入密码' : '請輸入密碼'"
...
// 品牌配置
const { brandName, locale } = useBrand()
    ...
    if (valid) {
      ...
      try {
        ...
        if (result.success) {
          ...
          ElMessage.error(result.message || (isSimplified.value ? '登录失败' : '登錄失敗'))
        }
      } catch (err: any) {
        ElMessage.error(err.message || (isSimplified.value ? '服务器不可用，请稍后重试' : '伺服器不可用，請稍後重試'))
      } finally {
        loading.value = false
      }
    }
