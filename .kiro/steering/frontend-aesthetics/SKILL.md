...
### MRPP（Mixed Reality Practical Platform）— 国内招投标/政企市场
- 风格：腾讯云活动页 cloud.tencent.com/act/pro 风格，深色背景 + 渐变光效 + 彩色卡片
- 字体：IBM Plex Sans（标题/数据）+ IBM Plex Mono（数字/代码）+ Noto Sans SC（正文）
- 配色：深蓝底（#080E1E / #0C1529），品牌蓝（#3B82F6），橙色强调（#FF6A00），成功绿（#10B981）
- Hero：深色背景 + 渐变文字 + HOT标签 + 橙色CTA按钮
- 卡片：深色底（#111B33）+ 蓝色边框 + hover顶部渐变线 + 圆角（16px）
- 背景：深蓝渐变 + 网格线 + 多色光球（蓝/紫/橙）
- 导航：深色毛玻璃 + Logo图标 + 蓝色品牌色
- 支持明暗双主题切换，默认深色，浅色模式白底蓝色调
- 左右布局场景展示 + 彩色标签 + 渐变数字高亮
